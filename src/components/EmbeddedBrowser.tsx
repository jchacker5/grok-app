...
export interface EmbeddedBrowserProps {
  ...
  /** Shows a close (×) button in the toolbar — for standalone/modal use. */
  onClose?: () => void;
}
...
export function EmbeddedBrowser({
  ...
  // Address bar + back/forward history. `url` is the *actively navigated*
  // location; `initialUrl` only seeds it and re-seeds it when the caller
  // hands us a genuinely new URL (e.g. a different chat link clicked while
  // this component instance is reused), tracked via `seededUrlRef`.
  const [url, setUrl] = useState(initialUrl);
  const [addressValue, setAddressValue] = useState(initialUrl);
  const [history, setHistory] = useState<string[]>([initialUrl]);
  const [historyIndex, setHistoryIndex] = useState(0);
  const seededUrlRef = useRef(initialUrl);

  useEffect(() => {
    if (initialUrl === seededUrlRef.current) return;
    seededUrlRef.current = initialUrl;
    setUrl(initialUrl);
  ...
  const navigateTo = (target: string) => {
    ...
    setUrl(withScheme);
    setAddressValue(withScheme);
    setHistory((h) => [...h.slice(0, historyIndex + 1), withScheme]);
    ...
    return (
      <div className={"embedded-browser " + className}>
        <div className="embedded-browser__bar">
          ...
          <form
            className="embedded-browser__addr-form"
            onSubmit={(e) => {
              e.preventDefault();
          ...
          {onClose ? (
            <button type="button" className="chrome-btn" onClick={onClose} title={tr("image.close")}>
              <IconClose size={14} />
            </button>
          ) : null}
        </div>
